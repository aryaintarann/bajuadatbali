@extends('layouts.web')

@section('isi')

    <!-- Page Header -->
    <div class="page-header">
        <h1>Detail Layanan</h1>
        <div class="breadcrumb-nav">
            <a href="{{ url('/') }}">Beranda</a>
            <span>/</span>
            <span>{{ $serv->nama_layanan }}</span>
        </div>
    </div>

    <div class="container-fluid px-xl-5 pb-5">
        <div class="row">

            <!-- Service Image -->
            <div class="col-lg-5 mb-4">
                <div class="card border-0 shadow-sm" style="border-radius:16px; overflow:hidden;">
                    <img class="w-100" style="object-fit:contain; max-height:450px; background-color:#f8f9fa;"
                        src="{{ asset('file/layanan/' . $serv->gambar_layanan) }}"
                        alt="{{ $serv->nama_layanan }}">
                </div>
            </div>

            <!-- Service Info -->
            <div class="col-lg-7 mb-4">
                <div class="card border-0 shadow-sm p-4" style="border-radius:16px;">

                    <span class="badge mb-3" style="background:#fff0ec; color:#E8500A; font-size:0.8rem; padding:6px 14px; border-radius:50px; font-weight:500; display:inline-block; width:fit-content;">
                        <i class="fas fa-concierge-bell mr-1"></i> Layanan Kami
                    </span>

                    <h2 style="font-family:'Playfair Display',serif; font-weight:700; color:#1a1a1a; margin-bottom:20px;">
                        {{ $serv->nama_layanan }}
                    </h2>

                    <div style="color:#555; line-height:1.9; font-size:0.95rem; margin-bottom:30px;">
                        {!! $serv->keterangan_layanan !!}
                    </div>

                    <hr style="border-color:#f0f0f0;">

                    <!-- Share -->
                    <div class="d-flex align-items-center">
                        <span style="color:#666; font-size:0.9rem; margin-right:12px; font-weight:500;">Bagikan:</span>
                        <a href="#" style="width:34px; height:34px; border-radius:50%; background:#3b5998; color:#fff; display:inline-flex; align-items:center; justify-content:center; margin-right:8px; font-size:0.85rem; text-decoration:none;">
                            <i class="fab fa-facebook-f"></i>
                        </a>
                        <a href="#" style="width:34px; height:34px; border-radius:50%; background:#1da1f2; color:#fff; display:inline-flex; align-items:center; justify-content:center; margin-right:8px; font-size:0.85rem; text-decoration:none;">
                            <i class="fab fa-twitter"></i>
                        </a>
                        <a href="#" style="width:34px; height:34px; border-radius:50%; background:#25d366; color:#fff; display:inline-flex; align-items:center; justify-content:center; font-size:0.85rem; text-decoration:none;">
                            <i class="fab fa-whatsapp"></i>
                        </a>
                    </div>
                </div>
            </div>

        </div>
    </div>

@endsection
